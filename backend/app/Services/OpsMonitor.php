...
class OpsMonitor
{
    ...
    private function staleBackup(CarbonImmutable $now): ?string
    {
        ...
        // Чи лежить копія ще десь, крім цього сервера, монітор свідомо не
        // перевіряє (рішення 2026-09-16): місце поза VPS поки не вибране, і
        // нагадування раз на 6 годин про те, чого зараз не виправити, лише
        // привчило б гортати сповіщення монітора не читаючи.
        return $hours >= self::BACKUP_STALE_HOURS
            ? "Свіжого бекапу бази немає {$hours} год. Перевірте cron під www-data і вивід ops/backup-db.sh."
            : null;
    }
