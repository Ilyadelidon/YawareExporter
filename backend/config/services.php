...
return [
    ...
    */
    ...
    'telegram' => [
        ...
        // Пошта розробника, якому в особистий Telegram дублюються технічні
        // сповіщення (стан автогенерації, збої планувальника). Адміністратори
        // підключають свій технічний чат у «Налаштуваннях»; порожнє значення
        // лишає сповіщення лише їм.
        'ops_email' => env('OPS_TELEGRAM_EMAIL'),
    ],

    'ops' => [
        // URL зовнішнього сигналу живості (healthchecks.io тощо). Сервіс пінгує
        // його щопівгодини; тривогу піднімає той бік, коли пінг не прийшов —
        // це єдиний спосіб дізнатися про смерть самого сервера.
    ...
    'bitrix' => [
        ...
        // Токени видає хмара Бітрікса, а не сам портал — адреса спільна для
        // всіх хмарних порталів (для «коробки» її задають окремо).
        'oauth_url' => env('BITRIX_OAUTH_URL', 'https://oauth.bitrix.info/oauth/token/'),
        // Куди повертати браузер після авторизації. У проді фронтенд лежить на
        // тому ж домені, що й API, тож дефолту APP_URL достатньо; у dev —
        // http://localhost:5173.
